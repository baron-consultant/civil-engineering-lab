...
- `npm run export:pdf`: 사이트를 빌드 후 섹션별(랜딩 + Civil DX/기반기술/설계/시공) 페이지를 기사 본문만 PDF로 병합해 `dist/pdf/civil-engineering-lab-<sha>.pdf`에 생성합니다. 헤더/사이드바 등 레이아웃은 제외되고, `[data-pagefind-body]`가 없는 페이지(404 등)와 help 폰트 데모 같은 비콘텐츠 페이지는 스킵합니다. 해상도는 뷰포트 2400×3200에 스케일 2로 출력합니다. 다운로드 아이콘 노출은 `PUBLIC_PDF_ICON_ENABLED=true`일 때만 표시되며, 링크는 `PUBLIC_PDF_DOWNLOAD_URL`을 사용합니다.
